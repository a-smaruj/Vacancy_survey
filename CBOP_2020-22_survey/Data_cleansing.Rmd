...
```{r}
create_table <- function(cbop_year, files, columns_to_keep) {
...
diff_n <- length(cbop_2020$hash) - n

#Table with duplicates and with unique value
cbop_duplicate <- cbop_2020 %>% get_dupes(hash)

cbop_unique <- setdiff(cbop_2020, cbop_duplicate[,-2,with=FALSE])

#Analysis - duplicates
e_duplicate <- e_duplicate %>% arrange(desc(hash))
tab_03141d <- e_duplicate %>% filter(hash == '03141d53cf41912a3d045fd56be42788')
...
#Chełm w różnych woj, powatach, gminach '6009fe476b0739ef6d3f8d9cb73e784d3'
#ulica i brak ulicy....
#war_opisWynagrodzenia '+ staz pracy  + 100% premii regulaminowej' lub '+ staz pracy  + 100% premii regulaminowej'
...
cbop_2022 <- create_table(cbop_2022, files, columns_to_keep)
...
cbop_2021_1 <- create_table(cbop_2021_1, files_1, columns_to_keep)
cbop_2021_1 <- remove_duplicates(cbop_2021_1)

cbop_2021_2 <- list()
files_2 <- files[(1+floor(length(files)/2)):length(files)]
cbop_2021_2 <- create_table(cbop_2021_2, files_2, columns_to_keep)
