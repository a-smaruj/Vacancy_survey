...
create_table <- function(cbop_year, files) {
  ...
  files <- str_remove(files, 'data\\\\\\d+\\\\')
  files <- str_remove(files, '_full.json')
...
d_unique <- cbop_2020 %>% filter(duplicated(cbop_2020, by='hash', 
                                     fromLast = F) == FALSE)

d_duplicate <- cbop_2020 %>% filter(duplicated(cbop_2020, by='hash', 
                                     fromLast = F) == TRUE)

d_duplicate <- d_duplicate %>% arrange(desc(hash))
d_duplicate %>% filter(hash == 'fcdb1d45049fbe8bdff2fb6b397fba5c')
```
